...
test('grain() is decoration over the content, not a layer in it', () => {
  assert.equal(ui.grain('x'), '<div class="su-grain">x</div>');
  assert.match(
    ui.grain({ opacity: 0.4, blend: 'overlay', as: 'section' }, 'x'),
...
test('the noise fills the box once, at one unit to the pixel', () => {
  ...
  const svg = /--su-grain-image: url\(&#34;data:image\/svg\+xml,(.+?)&#34;\)/.exec(ui.grain({ seed: 7 }, 'x'))[1];
...
test('grain() renders no tile of its own until asked', () => {
  ...
  for (const props of [{}, { opacity: 0.3 }, { blend: 'overlay' }, { seed: 0 }, { color: 'var(--brand)' }]) {
    assert.ok(
      !ui.grain(props, 'x').includes('--su-grain-image'),
...
test('the tile grain() builds is the one the stylesheet carries', () => {
  ...
  const inline = /--su-grain-image: (url\(&#34;.+?&#34;\))"/
    .exec(ui.grain({ seed: 7 }, 'x'))[1]
    .replaceAll('&#34;', '"')
    .replace("seed='7'", "seed='0'");

  assert.ok(ui.stylesheet({ minify: false }).includes(`--su-grain-image: ${inline};`));
});

test('grain() writes the noise props into the filter it builds', () => {
  assert.match(
    ui.grain({ type: 'turbulence', frequency: 0.2, octaves: 5, seed: 3 }, 'x'),
    /type='turbulence' baseFrequency='0.2' numOctaves='5' seed='3'/,
  );
  assert.match(ui.grain({ type: 'nonsense' }, 'x'), /type='fractalNoise'/, 'unknown type falls back');
  ...
  assert.match(ui.grain({ color: '#ff8800' }, 'x'), tint);
  assert.match(ui.grain({ color: '#f80' }, 'x'), tint);
  assert.match(ui.grain({ color: 'rgb(255, 136, 0)' }, 'x'), tint);
  assert.equal(ui.grain({ color: 'rebeccapurple' }, 'x'), ui.grain('x'), 'a colour it cannot read is left alone');
  assert.equal(ui.grain({ color: '#fff' }, 'x'), ui.grain('x'), 'white multiplies to nothing, so it is no tint');
  assert.equal(ui.grain({ color: '#ff880000' }, 'x'), ui.grain('x'), 'alpha zero is no tint either');
  assert.equal(ui.grain({ color: '#ffffff80' }, 'x'), ui.grain('x'), 'white at any alpha changes nothing');
  ...
  assert.match(ui.grain({ color: 'rgba(255, 136, 0, 0.5)' }, 'x'), half);
  assert.match(ui.grain({ color: 'rgb(255 136 0 / 50%)' }, 'x'), half);
  // 0x80 is 0.502 of 255, so the hex form lands a hair past the half.
  assert.match(ui.grain({ color: '#ff880080' }, 'x'), /values='1 0 0 0 0 0\.766 0 0 0 0 0\.498 /);

  // What the browser half reads back from a tile is what went in.
  const html = ui.grain({ type: 'turbulence', frequency: 0.2, octaves: 5, seed: 42, color: '#ff8800' }, 'x');
