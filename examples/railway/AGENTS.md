...
- UI components: `sitelo/ui` (buttons, cards, forms, tables, tabs, modals) — also plain functions returning HTML. `styles()` in `head()`; panel tabs, dismissible alerts, menus and the theme toggle import their own runtime from an inline `onclick`, so there is no client script to add. `sitelo/ui/client` exports `toast()` and the theme helpers for calling yourself. Do not add a React component library. See https://sitelo.dev/docs/ui
- Server islands: `island()` + `src/islands/<name>.js` + `mountIslands()`; `sitelo` / `sitelo preview` serve `/_sitelo/islands`; in production `server.js` serves `dist/` and renders `src/islands/` (Railway runs it with `npm start`).
