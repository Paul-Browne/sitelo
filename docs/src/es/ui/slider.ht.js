...
export default () =>
  ...
  help: 'Más alto pesa más y hace la compilación más lenta.',
      ...
      h2('Ponerlo desde tu propio código'),
      p(
        'Un deslizador es un control de formulario, así que es del lector — pero un preajuste, un botón de reinicio o un valor que llega por la red tienen que poder moverlo. Dale un ',
        code('id'),
        ' y ',
        code('setSlider'),
        ' lo hace, llevándose con él el ',
        code('<output>'),
        '.',
      ),
...
setSlider('volume', 50)`, 'javascript'),
      p('O alcanza el módulo como lo alcanzan los componentes, y sáltate el bundle por completo:'),
      codeBlock('En cualquier sitio', `button({ onclick: "import('/su/slider.js').then(m=>m.set('volume',50))" }, 'Mitad')`, 'javascript'),
      p(
        'El navegador recorta a ',
        code('min'),
        ' y ',
        code('max'),
        ' y ajusta al ',
        code('step'),
        ', así que lo que vuelve es dónde acabó el deslizador, no lo que se le pasó. Después van ',
        code('input'),
        ' y ',
        code('change'),
        ', porque una vista previa que escucha el arrastre no tiene otra forma de enterarse de un movimiento que no provocó. ',
        code('getSlider()'),
        ' vuelve a leer el valor.',
      ),

      h2('Pruébalo'),
      p('Esta página carga el runtime, así que los botones de abajo mueven el deslizador de verdad.'),
      demo(`stack({ gap: 'md' },
