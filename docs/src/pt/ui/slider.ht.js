...
export default () =>
  ...
  help: 'Mais alto é maior e mais lento a construir.',
      ...
      h2('Defini-lo a partir do teu código'),
      p(
        'Um cursor é um controlo de formulário, por isso é do leitor — mas uma predefinição, um botão de reposição ou um valor que chega pela rede têm de o conseguir mover na mesma. Dá-lhe um ',
        code('id'),
        ' e o ',
        code('setSlider'),
        ' fá-lo, levando o ',
...
setSlider('volume', 50)`, 'javascript'),
      p('Ou vai buscar o módulo como os componentes vão buscar o deles, e salta o bundle por completo:'),
      codeBlock('Em qualquer lado', `button({ onclick: "import('/su/slider.js').then(m=>m.set('volume',50))" }, 'Metade')`, 'javascript'),
      p(
        'O browser limita a ',
        code('min'),
        ' e ',
        ...
        ' e encaixa no ',
        code('step'),
        ', por isso o que volta é onde o cursor ficou, não o que lhe foi dado. A seguir vêm ',
        code('input'),
        ' e ',
        code('change'),
        ', porque uma pré-visualização à escuta do arrasto não tem outra forma de saber de um movimento que não causou. O ',
        code('getSlider()'),
        ' lê o valor de volta.',
      ),

      h2('Experimenta'),
      p('Esta página carrega o runtime, por isso os botões abaixo movem mesmo o cursor.'),
      demo(`stack({ gap: 'md' },
