...
export default () =>
  uiLayout({
    ...
    children: [
      ...
      p(
        '在办不到的地方，会换成一排真正的圆点接手——每张幻灯片一个链接，同样不用脚本。这种形态在两件具体的事上更弱，两件都写在下面，但它保证了手机拿到的轮播，绝不会没有任何东西说明它能滚。',
      ),
  ...
  carousel({ dots: false, items: ['只有箭头', '第二张', '第三张'].map((name) =>
    aspectRatio({ ratio: '16 / 5', style: 'background: var(--su-surface-2); border-radius: 0.6rem' },
      ...
      h2('当浏览器没有滚动标记时'),
      p(
        '这时圆点就是普通链接，每张幻灯片一个，各自指向自己那张的 id——每张幻灯片都拿到一个 id 就是为了这个。点一下圆点就把轨道滚到那里，什么都不用加载；而在有原生标记的地方这一排会重新隐藏，免得有人看到两排。',
      ),
      p(
        '有两件事它们做不到，而原生标记做得到。点开一个指向幻灯片的链接就是跟着一个片段走，所以除了轨道，窗口也会移向那张幻灯片——页面会滚动。',
        code('scrollMargin'),
        ' 决定它停在幻灯片上方多高的位置；给粘性页头至少留出它自己的高度。另外它们无法标出正在显示的是哪一张：',
        code(':target'),
        ' 跟得上点圆点这个动作，却完全不知道滑动这件事，而一个在你滑走的那一刻就过期的圆点，比一个从不声称什么的圆点更糟。所以它们是"去哪里"，不是"你在哪里"的写照。',
      ),
      p(
        '它们指向的 id 来自 ',
        code('name'),
...
h2('给幻灯片命名'),
  ...
  items: [
    ...
    { label: '花园', content: card({ variant: 'flat' }, cardBody(text({ align: 'center' }, '花园'))) },
      ...
      p(
        '没有原生标记时，它同样无法标出当前那一张，也无法在不移动页面的前提下跳到某一张——上面说的那两项代价。滑动、触控板和按键两种情况下都一样。',
      ),
      ...
      p(
        '后备圆点是链接，每个都以自己那张幻灯片命名，每个的点击目标是 24px，而不是圆点看上去的 8px。它们不带 ',
        code('aria-current'),
        '——那会在构建时写死一次，而轨道一被滑动就不再正确。在原生标记取代它们的地方，它们是 ',
        code('display: none'),
      ...
      propsTable([
        ...
        ['dots', 'boolean', 'true', '轨道下方的圆点——浏览器有原生滚动标记时用它，没有时每张幻灯片一个链接。关掉会让滚动条回来。'],
