...
// <link rel="stylesheet" href="/su/neumorphism-5d0e7b91.css">`, 'javascript'),
      ...
      p(
        code('glassmorphism'),
        ' 是磨砂玻璃：每个表面都是一块半透明的面板，把身后的东西模糊掉，边缘带着光。玻璃只有盖在什么东西上才像玻璃，所以它要求页面的背景是 ',
        code('var(--su-glass-ground)'),
        '——页面底色加上固定在后面的三团色彩。文字按面板可能覆盖的最亮颜色来检验对比度，浮在页面文字之上的东西都磨砂到几乎不透明，要求减少透明度的读者会看到同样的面板压在静止的底色上。',
      ),
  ...
  background: var(--su-glass-ground);
      ...
      p(
        code('theme()'),
        ' 依然可以叠在上面用，所以预设是起点，而不是分叉。两个预设都给每套调色板加了第十个槽位 ',
        code('glow'),
...
head(
  ...
  stack({ direction: 'row', gap: 'sm', wrap: true, align: 'flex-end' },
    ...['xs', 'sm', 'md', 'lg', 'xl'].map((step) =>
      ...
      p(code('styles()'), '：'),
      propsTable([
        ['preset', "'glassmorphism' | 'neumorphism'", '', '用预设重新设计每个组件，链接或内联在核心样式表之后。'],
