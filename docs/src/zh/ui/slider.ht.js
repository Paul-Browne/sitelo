...
export default () =>
  ...
  help: '数值越高，文件越大，构建也越慢。',
      ...
      h2('从你自己的代码里设置'),
      p(
        '滑块是表单控件，本该由读者掌控——但预设、重置按钮，或者从网络取回的数值，仍然得能移动它。给它一个 ',
        code('id'),
        '，',
        code('setSlider'),
        ' 就能做到，旁边的 ',
...
setSlider('volume', 50)`, 'javascript'),
      p('或者像各个组件取自己的模块那样去取它，完全不用打包：'),
      codeBlock('任意位置', `button({ onclick: "import('/su/slider.js').then(m=>m.set('volume',50))" }, '一半')`, 'javascript'),
      p(
        '浏览器会按 ',
        code('min'),
        ' 和 ',
        ...
        ' 夹住，并对齐到 ',
        code('step'),
        '，所以返回的是滑块最终停在哪儿，而不是你递给它什么。随后还会发出 ',
        code('input'),
        ' 和 ',
        code('change'),
        '，因为监听拖动的预览没有别的办法知道这次并非由它引起的移动。',
        code('getSlider()'),
        ' 则把值读回来。',
      ),

      h2('试一试'),
      p('本页加载了运行时，所以下面的按钮真的会移动滑块。'),
      demo(`stack({ gap: 'md' },
