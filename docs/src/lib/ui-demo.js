...
function cssBlocks(css) {
  ...
  while (i < css.length) {
    ...
    blocks.push({ prelude: css.slice(i, open).trim(), body: css.slice(open + 1, j - 1) })
    i = j
  }
...
/*
 * A preset written for a whole page, confined to one element.
 *
 * A preset sets its tokens on `:root` and restyles every `su-` class it
 * names — and this site's own chrome is built from the library, so on
 * the page as written it would restyle the bar, the sidebar and the
 * tables along with the example. Inside `@scope` its component rules
 * reach only what is in the stage, and its selectors are moved to match:
 *
 * - `:root` is the stage itself, so the tokens land on it and everything
 *   in it inherits them.
 * - A theme attribute the preset reads off `<html>` — the dark blocks —
 *   is looked for on the stage's ancestors, where the docs toggle writes
 *   it, and still on anything inside that carries its own.
 * - `@property` and `@keyframes` cannot be scoped, and are global anyway.
 */
function scopeSelectors(prelude) {
  return prelude
    .split(/,(?![^(]*\))/)
    .flatMap((selector) => {
      if (selector === ':root') return [':scope']
      if (selector.startsWith(':root')) return [`${selector} :scope`]
      if (/^\[data-(su-)?theme[^ ]*$/.test(selector)) return [`${selector} :scope`, selector]
      return [selector]
    })
    .join(',')
}
...
function scopeRules(css) {
  return cssBlocks(css)
    .map(({ prelude, body }) =>
      /^@(media|supports)/.test(prelude)
        ? `${prelude}{${scopeRules(body)}}`
...
/**
 * The core's tokens and nothing else of its sheet: every `--su-`
 * declaration in its light and dark blocks.
 *
 * @param {string} css
 * @returns {string}
 */
function coreTokens(css) {
  return cssBlocks(css)
    .map(({ prelude, body }) => {
      if (prelude.startsWith('@media (prefers-color-scheme: dark)')) {
        const inner = coreTokens(body)
        return inner && `${prelude}{${inner}}`
      }
      if (!/^(:root|\[data-(su-)?theme)/.test(prelude)) return ''
      const tokens = body.split(';').filter((declaration) => declaration.trim().startsWith('--su-'))
      return tokens.length ? `${prelude}{${tokens.join(';')}}` : ''
    })
    .join('')
}

/**
 * A preset's sheet, scoped to {@link PRESET_STAGE}.
 *
 * The core's own tokens go in first, scoped the same way. A site that
 * picks a preset has the core's defaults under it, and the preset only
 * restates what it changes; here the stage would otherwise inherit this
 * site's `uiTheme()` for everything the preset leaves alone — its soft
 * fills and its borders — and show the docs' green through the preset.
 *
 * @param {string} preset
 * @returns {string}
 */
function scopedPresetCss(preset) {
  // `stylesheet({ preset })` is the core and then the preset, and the
  // core is already on the page — so only the part after it is new.
  const core = ui.stylesheet()
  const css = ui.stylesheet({ preset }).slice(core.length)
  const blocks = cssBlocks(css)
  const global = blocks.filter(({ prelude }) => /^@(property|keyframes)/.test(prelude))
  const scoped = blocks.filter((block) => !global.includes(block))

  return (
    global.map(({ prelude, body }) => `${prelude}{${body}}`).join('') +
    `@scope (${PRESET_STAGE}){${scopeRules(
      coreTokens(core) + scoped.map(({ prelude, body }) => `${prelude}{${body}}`).join(''),
    )}}`
  )
}
...
export function createUiDemo(lang = DEFAULT_LOCALE) {
  ...
  function presetPreview(preset) {
    ...
    const gallery = ui.stack(
      ...
      ui.grid(
        ...
        ui.card(
          ...
          ui.cardFooter(
            ...
            ui.button({ block: true }, t.solid),
            ui.button({ variant: 'outline', block: true }, t.outline),
            ui.button({ variant: 'ghost', block: true }, t.ghost),
          ),
        ),
        ui.stack(
          { gap: 'lg' },
        ...
        ui.stack(
          ...
          // Never wrapped: a divider only divides what is either side of it.
          ui.stack(
            { direction: 'row', gap: 'sm', align: 'center' },
            ui.button({ ...soft, size: 'sm', endIcon: ui.icon('share') }, t.share),
            ui.divider({ orientation: 'vertical', spacing: 'none' }),
            ui.button({ ...soft, size: 'sm', startIcon: ui.icon('location') }, t.label),
          ),
        ),
      ),
      ui.input({ 'aria-label': t.search, placeholder: t.searchFor, startAdornment: ui.icon('search') }),
      // Buttons rather than links: nothing here to page through.
      ui.pagination({ page: 6, count: 12, siblings: 1 }),
      ...
      ui.grid(
        ...
        ui.stack(
          ...
          // into, held in the flow, round the markup `toast()` builds.
          ui.toasts(
            { class: 'preset-toasts' },
            ui.alert({ color: 'neutral', icon: false, dismissible: true, dismissLabel: t.dismiss }, t.copied),
          ),
        ),
      ),
      ui.grid(
        ...
        ui.stack(
          ...
          ui.toggleGroup({
            label: t.view,
            items: [t.list, t.grid, t.board],
            value: t.grid,
            // `true`, not a flip: a segment already chosen stays chosen,
            // and the module lets go of the rest of the group itself.
            onclick: `const b=event.target.closest('.su-toggle-btn');if(b)import('${RUNTIME}pressed.js').then(m=>m.set(b,true))`,
          }),
          ui.tabs({
            variant: 'pills',
            name: 'preset-tabs',
    ...
    return div(
      {
        class: 'ui-preset-stage',
        'data-preset-stage': preset,
        role: 'group',
        'aria-label': t.frame,
        'data-pagefind-ignore': '',
      },
  ...
  /**
   * What {@link presetPreview} needs in `<head>`: pass it as `extraHead`.
   *
   * @param {string} preset
   * @returns {string}
   */
  function presetPreviewHead(preset) {
    return `<style data-preset-scope="${preset}">${scopedPresetCss(preset)}</style>`
  }
  ...
  return { demo, propsTable, presetPreview, presetPreviewHead, grainSandbox, grainSandboxHead }
}
...
export const presetPreviewHead = en.presetPreviewHead
